...
- **학생 실시간 응시 (2026-09-03 추가).** 학생이 폰(마이페이지)에서 ①~⑤로 답을 제출하면 자동 채점되어 성적에 들어감. 흐름: 교사가 시험 상세에서 **"📝 답안 등록"으로 정답 입력 → "▶ 응시 시작"** → 학생 마이페이지 상단에 "📱 지금 응시할 수 있는 테스트" 카드가 뜸 → 학생이 답 선택 후 제출(한 번 제출하면 수정 불가) → 교사가 그 시험 상세를 열거나 "🔄 제출 답안 채점하기"/"■ 응시 마감"을 누르면 자동 채점되어 `scores`에 저장(등수·평균·등급도 기존 로직으로 자동 재계산).
  ...
  - **주관식(숫자 기입형) 문항 지원 (2026-09-06 추가).** 실시간 응시 답안 항목(`akRows`, `exam_keys.answerKey`의 각 원소)에 `type`(`'mc'`=객관식 기본값 / `'short'`=주관식) 필드 추가. session.html "답안 등록" 모달에서 문항마다 "객관식/주관식(숫자)" 토글 버튼으로 전환 가능 — 주관식이면 ①~⑤ 버튼 대신 정답 숫자를 입력하는 텍스트칸이 뜸(⚠ 이 문항은 OMR 스캔 채점 대상이 아님 — OMR은 지금도 5지선다 마킹만 읽으므로 주관식 문항이 섞인 시험을 OMR로 채점하면 그 문항만 항상 오답 처리됨, 폰 실시간 응시 전용). 정답 자체는 그대로 `exam_keys`에만 있고, 문항 타입(정답 아님)만 `exams.questionTypes` 배열로 공개해서 학생 화면이 문항별로 어떤 입력창을 그릴지 알 수 있게 함. 채점 시 "5"/"05"/"5.0"처럼 표기가 달라도 같은 숫자면 정답 처리되도록 숫자 변환 비교(`answerMatches`/firebase-api.js, `akAnswerMatches`/session.html — 두 파일에 같은 로직이 중복 존재, 채점 기준을 고칠 땐 둘 다 같이 고칠 것). 학생 화면(mypage.html)은 문항 타입에 따라 5지선다 버튼 또는 숫자 입력창을 그리고(`liveExamCur.questionTypes`), 입력한 답도 선택형과 동일하게 localStorage에 저장되어 창을 실수로 닫아도 유지됨.
