...
- **게임 ① 똥 피하기 (`default`)**: 위에서 떨어지는 💩를 좌우로 피하는 캔버스 게임, 터치 드래그/좌우 방향키 지원, 생존 시간이 곧 점수, 시간이 지날수록 난이도(장애물 속도/spawn 간격) 자동 상승. 노란 동그라미 안에 "복"(복습노트, 5점)/"워"(워크북, 7점) 글씨가 랜덤으로 같이 떨어지며 먹으면 보너스 점수(2026-09-06 추가, 원래는 ⭐ 별이었음). 충돌하면 게임 종료 후 자동으로 점수 제출.
...
- **게임 ② 단어 빨리치기 (`wordtype`, 2026-09-06 추가)**: 선생님이 admin.html "🎮 미니게임(단어) 문제관리" 탭에서 문제를 등록. **두 가지 등록 방식 지원**(`word_sentences` 컬렉션의 `type` 필드로 구분):
  - `type:'text'` — 문장을 직접 입력하면서 정답 단어를 `**정답**`처럼 별표 두 개로 감싸서 표시(`template` 필드에 원문 그대로 저장). 학생 화면은 그 별표 부분을 빈칸(`_____`)으로 바꿔서 보여줌(`parseWordTemplate`, game.html·admin.html에 동일 정규식 로직 중복 구현).
