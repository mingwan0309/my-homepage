...
- 과제 탭은 시험 탭과 완전히 동일한 목록/상세 구조(목록 카드 + "+ 과제 추가" + 클릭 시 상세에서 이름/날짜 수정·삭제). `homeworks`/`hw_status` 컬렉션 별도 사용.
  - **누적 그룹(trackName) (2026-09-03 추가).** `homeworks` 문서에 `trackName` 필드(비어있으면 과제 이름과 동일) 추가. 마이페이지 "누적성적표"(`api.getMyScoreHistory`)가 과제를 묶을 때 `hw.name`이 아니라 `hw.trackName`을 기준으로 그룹핑함 — "복습노트"/"복습노트 2차시"/"복습노트 제출"처럼 매주 과제 이름이 조금씩 달라지면 예전엔 누적성적표에서 전부 따로따로 떴는데, 이제 "누적 그룹"을 같은 값으로 지정해두면 하나로 이어서 보임. session.html "+ 과제 추가" 모달에서 이름칸 옆에 "누적 그룹" 칸을 각 행마다 따로 입력할 수 있고(비워두면 이름과 동일하게 자동 저장), 이미 만들어둔 과제도 상세 화면에서 "누적 그룹"을 나중에 수정할 수 있음(기존 과제도 소급 적용 가능). 시험(exams)에는 아직 이 기능이 없음(같은 문제가 있을 수 있으나 사용자가 숙제만 우선 요청함).
