...
- **하루 2회만 도전 가능** (2026-08-27 1회→2회로 완화) — `game_scores` 문서ID는 `{날짜}__{학생ID}__{랜덤}`으로 매 시도마다 새로 생성되고, `api.submitGameScore`가 그날 그 학생 문서 개수를 세어 `GAME_MAX_ATTEMPTS`(=2) 이상이면 거부. 서버(Firestore) 쪽에서 막는 구조라 새로고침/재접속으로 우회 불가. **테스트용 아이디 `1111`은 `GAME_UNLIMITED_IDS`에 포함되어 있어 하루 제한 없이 계속 도전 가능**(선생님 QA용) — 새 테스트 계정을 추가하려면 firebase-api.js의 이 배열에 아이디만 추가하면 됨.
