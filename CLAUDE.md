...
- **학생 상세 슬라이드 패널 + 학생 화면 미리보기 (2026-08-04 추가, 2026-08-05 조교도 사용 가능하게 확장).** admin.html "학생" 탭 목록에서 학생 이름 클릭 → 오른쪽에서 `.sd-panel` 슬라이드 패널이 열리며 기본정보 + 4개 탭(수강 이력/클리닉·상담 이력/질문 이력/성적 이력)을 보여줌(`openStuDetail`, API `getStudentFullHistory` — attendance/clinic_bookings/mandatory_clinic/qna/scores를 한 번에 조인해서 반환). "학교 성적"(내신 등)은 이 시스템에 그런 데이터가 없어서 구현 안 함. `mypage.html?previewStudentId=` 학생 화면 미리보기도 조교 로그인에 허용됨 — Firestore 규칙의 `attendance`/`scores`/`hw_status`/`material_views`/`quiz_scores` read에 `isAssistant()`가 추가되어 있어야 함(쓰기 권한은 조교에게 안 줌 — 조회만 가능, 학생 대신 조작은 교사만).
