...
- `mandatory_clinic`: 의무클리닉(수업 전 1시간, 정규 시간이 아닌 다른 요일/시간에 오는 학생 명단). id, studentId, name, type('regular'=매주 반복되는 정규 시간 변경 / 'temp'=이번 주만 특정 날짜 하루만 임시 변경), date(type이 temp일 때만 사용, 특정 날짜), day(실제로 오는 요일 — temp면 date에서 자동 계산됨), targetDay(이 클리닉이 어느 요일 몫인지 — 등록할 때 선생님이 매번 직접 선택, 예: 월/금반인데 수요일에 왔으면 "월요일 보충"인지 "금요일 선행"인지), time, memo, attendance(날짜별 출석/결석 맵, 예: `attendance["2026-08-02"]="결석"`), attendanceBy(같은 날짜 키로 누가 체크했는지 이름 기록 — 교사/조교 구분용, 학생 화면에는 노출 안 함), createdAt. 목록 필터링 시 type='temp'면 date가 오늘과 일치할 때만, type='regular'(또는 미지정 — 구버전 데이터 하위호환)면 day가 오늘 요일과 일치할 때만 노출됨. **추가로 targetDay가 오늘 요일과 일치하는 등록(= 오늘이 원래 이 학생 몫인데 다른 요일로 옮겨서 실제로는 오늘 안 오는 경우)도 참고용으로 같이 노출됨(2026-08-27 추가)** — 이런 항목은 흐리게 표시되고 "↪ 오늘 아님, OO요일에 옴" 배지가 붙으며, 출석/결석 체크·재촉·귀가·점수입력 버튼은 안 뜨고 교사의 삭제 버튼만 남음(오늘 실제로 오는 학생이 아니므로 출결 체크 대상이 아님 — 조교가 헷갈리지 않게 하기 위한 참고 정보). admin.html "의무클리닉" 탭에서 등록/출결체크(교사+조교 접근 가능). **삭제 버튼은 교사만 보임**(조교는 출석/결석 체크만 가능, 등록 자체를 지울 수 없음 — `cu.role==='teacher'` 가드). 결석 체크 시 확인창 후 **실제 카카오 알림톡이 학생·학부모 번호로 자동 발송됨**(아래 솔라피 연동 재사용, 둘 다 전화번호 없으면 발송 대신 경고 알림). 또한 session.html의 "수업 결과 발송" 메시지 작성창에서, 발송 대상 중 의무클리닉 등록된 학생이 있으면 **{의무클리닉} 데이터 삽입 토큰이 추가로 나타나** 해당 학생의 오늘 출결 상태를 문구에 자동으로 넣을 수 있음(`mcListSession` 전역변수, `computeMsgTokens`/`renderMsgTokenBar`). **학생 본인은 mypage.html에서 자기 의무클리닉 등록 현황을 볼 수 있음**(`getMyMandatoryClinic` API, "⏰ 의무클리닉" 카드 — 해당 없으면 카드 자체가 안 뜸).
  - **"오기 1시간 전" 자동 알림 (2026-09-04 추가).** 시간대가 바뀐(오늘 실제로 오는) 학생에게, 등록해둔 `time` 기준 1시간 전 즈음 자동으로 카카오 알림톡이 감. **서버에서 예약 실행되는 게 아니라, admin.html을 켜둔 사람(교사든 조교든 아무나)이 있는 동안에만 작동함** — `startMcHourReminderTimer()`가 페이지 로드 시 시작돼서 1분마다 `checkMcHourReminders()`로 확인. 시각 계산은 `time` 필드를 느슨하게 파싱(`mcParseTimeToMinutes`, "17:00"/"17시" 등)해서 지금 시각과 비교, 도착까지 50~60분 남은 구간에서 발송. 중복 발송 방지는 `mandatory_clinic.hourReminderSent[날짜]`에 표시(`api.markMcHourReminderSent`)해서 확인 — **단, 교사와 조교가 동시에 화면을 켜두고 있으면 아주 드물게 같은 60초 틱에 겹쳐 중복 발송될 수 있음(원자적 잠금 아님, 알려진 한계)**. 별도 서버/Apps Script 예약 트리거 없이 기존 클라이언트 구조 그대로 구현한 것이라, 아무도 admin.html을 켜놓지 않은 시간대(예: 새벽)에는 당연히 안 감.
