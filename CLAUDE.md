...
- "비밀번호 변경(선생님이 대신)"과 "메시지 발송(SMS/카톡)"은 의도적으로 안 만듦 — 위 로그인 항목의 기술적 한계, 그리고 메시지 발송은 유료 SMS/카카오 API 연동이 필요해서 별도 논의 없이는 구현 안 함.
...
- 과제의 실제 결과(통과/미통과)는 **과제 탭이 아니라 성적 탭 표**에 시험 칼럼 오른쪽으로 붙어서 표시됨. 셀 클릭 시 시험과 별개의 패널(hw-panel)에서 통과/미통과+코멘트 입력 (점수 입력 없음).
