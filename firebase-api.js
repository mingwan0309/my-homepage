...
api.submitExamAnswers = function(db, p){
  ...
  return db.collection('exams').doc(eid).get().then(function(ex){
    ...
    return db.collection('exam_submissions').where('examId','==',eid).where('studentId','==',sid).get().then(function(qs){
      ...
      // 제출 자체(핵심 데이터)는 먼저 확실히 저장한다 — 자기채점 저장이 막히더라도 제출은 절대 실패하면 안 됨.
      return db.collection('exam_submissions').doc(docId).set({
        ...
        return db.collection('exam_keys').doc(eid).get().then(function(keyDoc){
          ...
          wrongQuestions.sort(function(a,b){ return a-b; });
          score = Math.round(score*10)/10; total = Math.round(total*10)/10;
          // 새로고침 후에도 "제출 완료" 카드에 점수·틀린 문항을 계속 보여주기 위해 자기채점 결과를 제출 문서에 저장 시도.
          // 이 update가 보안 규칙상 막혀 있어도(권한거부) 제출 자체는 이미 성공했으므로 무시하고 넘어간다.
          return db.collection('exam_submissions').doc(docId).update({ selfScore:score, selfTotal:total, selfWrongQuestions:wrongQuestions }).catch(function(err){
            console.error('[submitExamAnswers] 자기채점 결과 저장 실패(제출 자체는 성공, Firestore 규칙에 exam_submissions update 허용 필요):', err);
        ...
        console.error('[submitExamAnswers] 제출 저장 실패:', err);
...
// 자료실 폴더(영상 자료/PDF 자료/손필기 자료 공통) — 아직 자료를 하나도 안 넣은 '빈 폴더'도
// 미리 만들어둘 수 있게 별도 컬렉션으로 관리. 실제 자료는 materials 문서의 folder 필드(폴더 이름)로만
// 연결되고, 이 컬렉션은 폴더 자체의 존재/이름/카테고리만 기록함(카테고리별로 폴더 목록이 안 섞이게).
api.addVideoFolder = function(db, p){
  var id = genId('vfolder');
  return db.collection('video_folders').doc(id).set({
    id:id, classId:String(p.classId), name:p.name||'', category:p.category||'영상 자료', createdAt:nowStr()
  }).then(function(){ return { success:true, id:id }; });
};
api.getVideoFolders = function(db, p){
  return db.collection('video_folders').where('classId','==',String(p.classId)).get().then(function(snap){
    var folders = docsToArr(snap)
      .filter(function(r){ return !p.category || (r.category||'영상 자료')===p.category; })
      .map(function(r){ return { id:String(r.id), name:r.name||'', category:r.category||'영상 자료' }; });
